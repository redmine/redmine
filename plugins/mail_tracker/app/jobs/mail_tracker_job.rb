...
class MailTrackerJob < ApplicationJob
  ...
  def parse_email(email, content_part, email_body)
    ...
    begin
      ...
      Sentry.capture_exception(e)
      log_string = "General error message id: #{email.message_id}, From: #{email.from}, To: #{email.to}, Subject: #{email.subject}, Date: #{email.date}, Error message: #{e}, Issue params: #{@issue_params}, Trace: #{e.backtrace}"
      MailTrackerCustomLogger.logger.error(log_string)
    ensure
      ...
      MailTrackerCustomLogger.logger.info(log_string)
      MailTrackerCustomLogger.logger.info('***')
      @issue = nil
      @issue_params = nil
  ...
  def issue_params(email, content)
    @issue_params = {
      ...
      "author_id": User.find_by_login(@mail_tracking_rule&.login_name).try(:id) || @mail_source.default_user_id,
