...
class IssueTest < ActiveSupport::TestCase
  ...
  def test_visible_scope_for_member
    user = User.find(9)
    # User should see issues of projects for which they have view_issues permissions only
    Role.non_member.remove_permission!(:view_issues)
  ...
  def test_visible_scope_for_admin
    ...
    issues = Issue.visible(user).all
    assert issues.any?
    # Admin should see issues on private projects that they do not belong to
