...
class ChangesetNotFound < RuntimeError; end
class InvalidRevisionParam < RuntimeError; end
...
class RepositoriesController < ApplicationController
  ...
  def committers
    ...
    if request.post? && params[:committers].is_a?(Hash)
      ...
      @repository.committer_ids = params[:committers].values.each_with_object({}) { |c, h| h[c.first] = c.last; h }
  ...
  def add_related_issue
    ...
    @changeset.issues << @issue if @issue
  end
  ...
  def find_project_repository
    ...
    unless @rev.to_s.match(REV_PARAM_RE) && @rev_to.to_s.match(REV_PARAM_RE)
      raise InvalidRevisionParam if @repository.branches.blank?
    end
  rescue ActiveRecord::RecordNotFound
    render_404
